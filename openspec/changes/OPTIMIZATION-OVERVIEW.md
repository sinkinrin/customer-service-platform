...
以下功能已从项目中移除，相关任务**不再有效**：

| 功能 | 状态 | 说明 |
|------|------|------|
| **Staff Conversations 页面** | ❌ 已删除 | `src/app/staff/conversations/` 目录不存在 |
| **人工转接(Human Agent)** | ❌ 已删除 | Customer 对话已简化为纯 AI 模式 |
| **SSE 实时更新系统** | ❌ 未实现 | `src/lib/sse/` 目录不存在 |
| **对话升级工作流** | ❌ 已删除 | 无 escalation 相关代码 |
| **队列管理系统** | ❌ 未实现 | 无 queue 相关页面 |

### 已完成的功能

参考 `enhance-ticket-system-features/tasks.md` 获取已完成功能列表。

---

## 📋 执行摘要 (历史记录)

基于2025年11月12日的全面代码审查和实际用户测试（使用Playwright），我们识别出了两个门户的多个关键Bug和功能缺失。

### ~~关键发现~~ (已过期)

| 门户 | ~~完成度~~ | ~~关键问题~~ | 说明 |
|------|--------|---------|--------|
| **Staff Portal** | ~~60%~~ | ~~Conversations崩溃~~ | Conversations 功能已删除 |
| **Customer Portal** | ~~75%~~ | ~~对话升级~~ | 已简化为 AI-only 模式 |
...
## 🔴 Staff Portal 优化 (⚠️ 部分过期)

### ~~核心问题~~ (已过期 - 功能已删除)

1. ~~**Conversations页面崩溃** (P0)~~ ❌ **功能已删除**
   - ~~错误：`TypeError: conversations.filter is not a function`~~
   - ~~位置：`src/app/(staff)/conversations/page.tsx:108`~~
   - **状态**: Staff Conversations 页面已从项目中移除

2. **Tickets路由** ✅ **已修复**
   - ~~问题：工单ID从60097截断为97~~
   - 当前状态：工单详情页正常工作

3. **Knowledge Base缺失** ⚠️ **仍然缺失**
   - URL：`/staff/knowledge` → 404
   - 状态：可作为独立需求实现

4. ~~**SSE连接不稳定** (P0)~~ ❌ **功能未实现**
   - ~~错误：Heartbeat timeout~~
   - **状态**: SSE 系统从未实现，已放弃此方案

### ~~优化计划~~ (大部分已过期)

#### ~~阶段1：紧急修复~~ (已过期)
- [x] 分析Bug原因
- ~~[ ] 修复Conversations页面~~ ❌ 功能已删除
- [x] 修复Tickets路由 ✅
- [ ] 创建Knowledge Base页面 ⚠️ 可独立立项
- ~~[ ] 修复SSE连接~~ ❌ 功能未实现

#### ~~阶段2：核心功能~~ (部分完成)
- [x] 工单分配系统 ✅ API 已实现
- [x] 响应模板系统 ✅ API 已实现（UI待完善）
- ~~[ ] 对话升级工作流~~ ❌ 功能已删除
- [ ] 文件上传功能 ⚠️ 后端有，前端集成缺失
...
~~- [ ] Conversations加载成功率 = 100%~~ ❌ 功能已删除
~~- [ ] SSE连接稳定性 > 99%~~ ❌ 功能未实现

[详细信息 →](./staff-portal-optimization/) ⚠️ 已归档

---

## 🟡 Customer Portal 优化 (⚠️ 部分过期)

### 核心问题 (状态更新)

1. **工单路由** ⚠️ **需验证当前状态**
   - ~~问题：跳转到`/staff/tickets/{id}`而非`/my-tickets/{id}`~~
   - 位置：`src/app/(customer)/my-tickets/page.tsx`
   - 状态：需要验证是否已修复
...
3. **文件上传未实现** ⚠️ **仍然缺失**
   - TODO：`src/app/(customer)/my-tickets/create/page.tsx:89-90`
   - 影响：无法提交附件
   - **这是当前 P0 优先级任务**

4. **FAQ评分系统** ✅ **API 已完成**
   - API：`src/app/api/faq/[id]/rating/route.ts` 已实现
   - 状态：UI 集成可能需要验证
