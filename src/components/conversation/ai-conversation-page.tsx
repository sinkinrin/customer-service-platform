...
export function AiConversationPage({
  ...
  const buildConversationHref = useCallback(
    (id: string, query?: string) => (query ? `${normalizedBasePath}/${id}?${query}` : `${normalizedBasePath}/${id}`),
    [normalizedBasePath]
  )
    ...
    if (conversationId) {
      if (isDraftConversation) {
        ...
        if (materializedDraftConversationId) {
          ...
          router.replace(buildConversationHref(materializedDraftConversationId))
        ...
        try {
          ...
          if (shouldApplyNewMarker) {
            try {
              ...
              next.delete('new')
              router.replace(buildConversationHref(conversationId, next.toString() || undefined))
              return
            } catch (error) {
              if (!isActive()) return
  ...
  const handleSelectHistoryConversation = useCallback((id: string) => {
    ...
    activeStreamingMessageIdRef.current = null
    abortStreamingRequest()
    router.push(buildConversationHref(id))
    setHistoryOpen(false)
  }, [abortStreamingRequest, buildConversationHref, conversationId, router])
  ...
  const handleAIMessage = async (content: string) => {
      ...
      if (isDraftConversation && !materializedConversationId) {
        const materializeRes = await fetch('/api/conversations', {
          ...
          body: JSON.stringify({
            ...
            initial_metadata: { aiMode: true, role: humanMessageRole, aiChatMode },
          }),
        })
        ...
        const persistRes = await fetch(`/api/conversations/${resolvedConversationId}/messages`, {
          ...
          body: JSON.stringify({
            content: trimmedContent,
            message_type: 'text',
            metadata: { aiMode: true, role: humanMessageRole, aiChatMode }
          }),
        })
    ...
    try {
      ...
      if (shouldSyncRouterToMaterializedConversation && resolvedConversationId && isCurrentSendContextActive(resolvedConversationId)) {
        ...
        router.replace(buildConversationHref(resolvedConversationId))
      }
  ...
  const displayMessages = aiMessages.map((msg) => ({
    ...
    sender: {
      ...
      role: msg.role === 'user' ? humanMessageRole : ('staff' as const),
    },
  }))
