...
const MarkdownMessage = React.lazy(() =>
  import("./markdown-message").then((module) => ({
    default: module.MarkdownMessage,
  })),
);

const ImageLightbox = React.lazy(() =>
  import("@/components/ui/image-lightbox").then((module) => ({
      ...
      <div className="flex-1 p-6 space-y-8">
        {[1, 2, 3].map((i) => (
          ...
          >
            ...
            <Skeleton
              className={cn("h-14 rounded-2xl", i % 2 === 0 ? "w-48" : "w-56")}
            />
            {i % 2 === 0 && (
              <Skeleton className="h-9 w-9 rounded-full flex-shrink-0" />
            )}
  ...
  return (
    <div className="space-y-1 py-4">
      {groupedMessages.map((group) => {
        ...
        const isCustomerMessage = senderRole === "customer" || message.sender_id === "user";
