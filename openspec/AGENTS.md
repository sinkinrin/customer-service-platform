...
AI 编程助手使用 OpenSpec 进行规范驱动开发的指令。
...
- 并且包含以下之一：`create`，`plan`，`make`，`start`，`help`
...
2. 选择唯一的动词开头的 `change-id` 并在 `openspec/changes/<id>/` 下脚手架 `proposal.md`，`tasks.md`，可选的 `design.md` 和规范增量。
...
- [ ] 检查 `changes/` 中的待决变更是否有冲突
- [ ] 阅读 `openspec/project.md` 了解约定
- [ ] 运行 `openspec list` 查看待决变更
...
```bash
# 基本命令
openspec list                  # 列出待决变更
openspec list --specs          # 列出规范
openspec show [项目]           # 显示变更或规范
openspec validate [项目]       # 验证变更或规范
openspec archive <change-id> [--yes|-y]   # 部署后归档（添加 --yes 用于非交互式运行）

# 项目管理
openspec init [路径]           # 初始化 OpenSpec
openspec update [路径]         # 更新指令文件
...
# 调试
openspec show [变更] --json --deltas-only
openspec validate [变更] --strict
```
...
**Migration**: [处理方式]
```
如果多个功能受到影响，为每个功能在 `changes/[change-id]/specs/<功能>/spec.md` 下创建多个增量文件——每个功能一个。
...
### 需求措辞
- 使用 SHALL/MUST 表示规范性需求（除非有意使用非规范性的 should/may）
...
- ADDED：引入可以独立存在的新能力或子能力。当变更是正交的（例如，添加"斜杠命令配置"）而不是改变现有需求的语义时，更倾向于 ADDED。
- MODIFIED：改变现有需求的行为、范围或验收标准。始终粘贴完整的更新后需求内容（标题 + 所有场景）。归档器将用您在此处提供的内容替换整个需求；部分增量将丢失之前的详细信息。
- RENAMED：仅当名称变更时使用。如果您还改变了行为，请使用 RENAMED（名称）加上 MODIFIED（内容）引用新名称。

常见陷阱：使用 MODIFIED 添加新关注点而不包含之前的文本。这会在归档时导致细节丢失。如果您没有明确改变现有需求，请在 ADDED 下添加新需求而不是这样做。

正确编写 MODIFIED 需求：
1) 在 `openspec/specs/<功能>/spec.md` 中定位现有需求。
...
- 更倾向于动词前缀：`add-`，`update-`，`remove-`，`refactor-`
